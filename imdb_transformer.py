...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        print ("pe.shape111111111")
        print (pe.shape) 
        # 在这个地方，有一个较为奇怪的点，其中输出的pe.shape的值将会是一个固定的值，但是其中的相加部分，要求其可以与可变的值x进行相加
        self.register_buffer('pe', pe)

    def forward(self, x):
        print ("x.shape111111111")
        print (x.shape)
        x = x + Variable(self.pe[:x.size(0), :],requires_grad=False)
        return x
    #这一句发生错误The size of tensor a (6273) must match the size of tensor b (128) at non-singleton dimension 0
...
if __name__ == '__main__':
    ...
    result_output.to_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\result\\transformer.csv", index=False, quoting=3)
    logging.info('result saved!')
#RuntimeError: The size of tensor a (6879) must match the size of tensor b (128) at non-singleton dimension 0
