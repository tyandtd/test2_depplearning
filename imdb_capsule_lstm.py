...
#在这个地方还是一个两层结构的一个，作为第二层的一个模块
class Capsule(nn.Module):
    # 在这个地方，默认dim_capsule=5，但是我现有的数据，不能满足他等于5,其必须和batch_size一致，在这个地方，输出会固定，
    # 最后经过测试，在输入为32时，明明两者输入size相同，但是还是不可以完成对输出的一个问题，会出现32不能匹配8的一个问题，
    # 但是，明明就是两个输入都是32啊，后面尝试将其改为5，能继续运行，然后尝试将其变大，让size变成20
    # 但是不知道是不是模型的问题，最后收敛的效果有一点太差了
    def __init__(self, num_hiddens, bidirectional, num_capsule=batch_size, dim_capsule=-1, routings=4, **kwargs):
    ...
    def forward(self, inputs):
        ...
        u_hat_vecs = u_hat_vecs.view((batch_size, input_num_capsule,
                                      self.num_capsule, self.dim_capsule))#先修正了结构
        #在这个模块的这里，
        # print("u_hat_vecs.shape")
        # print(u_hat_vecs.shape)
        ...
        # print("outputs.shape")
        # print(outputs.shape)
        return outputs  # (batch_size, num_capsule, dim_capsule) ，输出这些项是没有什么问题的，但是最后的匹配出现了问题
...
class SentimentNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        states, hidden = self.encoder(embeddings.permute(1, 0, 2))#时序
        # print("states.shape")
        # print(states.shape)
        capsule = self.capsule(states)#胶囊
        # print("capsule.shape")
        # print(capsule.shape)
        encoding = torch.cat([capsule[0], capsule[-1]], dim=1)#
        # print("encoding.shape2")
        # print(encoding.shape)
        outputs = self.decoder(encoding)
        # print("outputs.shape2")
        # print(outputs.shape)
        return outputs
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=len(train_iter), desc='Epoch %d' % epoch) as pbar:
            for feature, label in train_iter:
                ...
                # print(label.size())
                # print(label[1])
                # loss = loss_function(score, targets_m)
                #原本代码
    ...
    result_output.to_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\result\\capsule_lstm.csv", index=False, quoting=3)
    logging.info('result saved!')
#在154行报错，表示输入格式不符合对应格式