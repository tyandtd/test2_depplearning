...
#这个地方读取文件，没有读取pickle文件
train = pd.read_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\labeledTrainData.tsv", header=0, delimiter="\t", quoting=3)
test = pd.read_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\testData.tsv", header=0, delimiter="\t", quoting=3)

#在这个地方重新定义了两个类的模式，导致后面处理，一边是不用定义文件格式的，一边是不用新定义类，但是在最后两边的一个格式应该是一致的
...
#在这个文件里面，是用的最基础的格式做的，，没有定义一个metric函数
#其他的两个类似使用bert的训练，都直接导入trainer就开始训练了
    ...
    result_output.to_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\result\\bert_native.csv", index=False, quoting=3)
    logging.info('result saved!')
#完成一次，在result中生成了bert_native.csv文件