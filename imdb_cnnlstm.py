...
class SentimentNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #总体一共有两次位置的调换
        # cnn
        convolution = self.activate(self.conv1d(embeddings.permute([0, 2, 1])))#在这个地方加上了一个一维的卷积
        pooling = F.max_pool1d(convolution, kernel_size=pooling_size)#一个二取最大值的max函数
...
if __name__ == '__main__':
    ...
    result_output.to_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\result\\cnnlstm.csv", index=False, quoting=3)
    logging.info('result saved!')
#完成一次，在result中生成了cnnlstm.csv文件
