...
num_hiddens = 120
num_layers = 2
bidirectional = True#这个地方依旧是ture
batch_size = 64
labels = 2
lr = 0.8#疑惑，这个lr是不是有一点太高了
device = torch.device('cuda:0')
use_gpu = True

#定义类，其中的gru是一种类似于lstm的技术，甚至在两边要求定义输入的参数个数都一样，使用原因是其较为需要更行的参数数量较少
...
if __name__ == '__main__':
    ...
    result_output.to_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\result\\gru.csv", index=False, quoting=3)
    logging.info('result saved!')
#完成一次，在result中生成了gru.csv文件
