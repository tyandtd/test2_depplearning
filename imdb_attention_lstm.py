...
class Attention(nn.Module):
    def __init__(self, num_hiddens, bidirectional, **kwargs):
        ...
        # if bidirectional, then double the hidden dimensionality在许多模块中，都有定义这个双向模块的
        if self.bidirectional:
            self.w_omega = nn.Parameter(torch.Tensor(num_hiddens * 2, num_hiddens * 2))#新建了一个参数列表
        ...
        nn.init.uniform_(self.w_omega, -0.1, 0.1)# 均匀形成
        nn.init.uniform_(self.u_omega, -0.1, 0.1)

    def forward(self, inputs):
        x = inputs
        u = torch.tanh(torch.matmul(x, self.w_omega))#做乘法，其中这个有一点像短续时间模块
...
if __name__ == '__main__':
 ...
 # 训练过程
    for epoch in range(num_epochs):
        ...
        with tqdm(total=len(train_iter), desc='Epoch %d' % epoch) as pbar:
            for feature, label in train_iter:
                ...
                score = net(feature)
                loss = loss_function(score, label)#通过计算两者的损失
                loss.backward()
                optimizer.step()#开始迭代
    ...
    result_output.to_csv("F:\\deeplearning\\test2\\imdb_sentiment_analysis_torch\\result\\attention_lstm.csv", index=False, quoting=3)
    logging.info('result saved!')
#完成一次，在result中生成了attention_lstm.csv文件
